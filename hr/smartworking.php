...
<div class="app-layout">
  ...
  <main class="main-content">
    <div class="page-header"><h1>Gestione Smartworking</h1></div>
    ...
    <div style="display:flex;gap:1rem;margin-bottom:1.25rem;flex-wrap:wrap">
      ...
      <div class="card" style="flex:1;min-width:120px;text-align:center;padding:.75rem 1rem">
        <div style="font-size:.75rem;color:var(--text-light);text-transform:uppercase;letter-spacing:.04em">Giorni tot.</div>
...
async function loadCounters() {
  try {
    ...
    document.getElementById('cnt-days').textContent     = all.filter(r=>r.stato==='approved').reduce((s,r)=>s+(r.giorni||0),0);
  } catch(_) {}
}
...
function renderTable(reqs, tbl) {
  ...
  for (const r of reqs) {
    ...
    const fine = r.data_fine !== r.data_inizio ? formatDate(r.data_fine) : '—';
    html += `<tr style="border-bottom:1px solid var(--border);font-size:.85rem">
      ...
      <td style="padding:.5rem .75rem;color:var(--text-light);max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(r.motivo||'—')}</td>
...
function openApprove(id, empName) {
  showModal(`
    <div class="modal-header">
      <span class="modal-title">&#10003; Approva smartworking</span>
    ...
    <div class="modal-body">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" id="_app-note" placeholder="es. Approvato, buon lavoro!">
      </div>
  ...
  document.getElementById('_app-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      closeModal();
      showToast('Smartworking approvato ✓', 'success');
      loadCounters(); load();
...
function openReject(id, empName) {
  showModal(`
    <div class="modal-header">
      <span class="modal-title">&#x2715; Rifiuta smartworking</span>
      <button class="modal-close">&#x2715;</button>
    </div>
    <div class="modal-body">
      <p style="margin-bottom:1rem">Rifiutare la richiesta di <strong>${empName}</strong>?</p>
      <div class="form-group">
        <label class="form-label">Motivazione <span style="color:var(--danger)">*</span></label>
        <input type="text" class="form-control" id="_rej-note" placeholder="es. Troppi colleghi assenti in quel periodo…">
      </div>
...
['_fil-stato','_fil-emp'].forEach(id =>

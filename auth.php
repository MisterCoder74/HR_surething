 ...
 * require_once at top of every page/API.
 */
...
if (isset($_SESSION['last_activity'])) {
    if (time() - $_SESSION['last_activity'] > SESSION_TIMEOUT_HOURS * 3600) {
        ...
        if ($is_api) { http_response_code(401); header('Content-Type: application/json'); echo json_encode(['error'=>'Sessione scaduta']); exit; }
...
function validate_csrf(string $t): bool { return !empty($_SESSION['csrf_token']) && hash_equals($_SESSION['csrf_token'],$t); }
...
function require_hr(): void {
    require_login();
    if (!is_hr()) { http_response_code(403); include __DIR__.'/error_403.php'; exit; }
}
function require_employee(): void {
    require_login();
    if (!is_employee()) { http_response_code(403); include __DIR__.'/error_403.php'; exit; }
}
function api_require_login(): void {
    if (!is_logged_in()) { http_response_code(401); header('Content-Type: application/json'); echo json_encode(['error'=>'Non autenticato']); exit; }
}
function api_require_hr(): void {
    api_require_login();
    if (!is_hr()) { http_response_code(403); header('Content-Type: application/json'); echo json_encode(['error'=>'Accesso non autorizzato']); exit; }
}
function api_require_employee(): void {
    api_require_login();
    if (!is_employee()) { http_response_code(403); header('Content-Type: application/json'); echo json_encode(['error'=>'Accesso non autorizzato']); exit; }
}
function api_validate_csrf(): void {
    $t = $_POST['csrf_token'] ?? ($_SERVER['HTTP_X_CSRF_TOKEN'] ?? '');
    if (!validate_csrf($t)) { http_response_code(403); header('Content-Type: application/json'); echo json_encode(['error'=>'Token CSRF non valido']); exit; }
}
